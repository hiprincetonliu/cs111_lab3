...
struct hash_table_v1 *hash_table_v1_create()
{
	...
	int rc = pthread_mutex_init(&hash_mutex, NULL);
	if (rc != 0) {
		free(hash_table);
		exit(rc);
	}
...
void hash_table_v1_add_entry(struct hash_table_v1 *hash_table,
                             ...
                             uint32_t value)
{
	int rc = pthread_mutex_lock(&hash_mutex); // CHECK FOR ERRORS
	if (rc != 0) {
		exit(rc);
	}
	...
	rc = pthread_mutex_unlock(&hash_mutex);
	if (rc != 0) {
		exit(rc);
	}
}
...
void hash_table_v1_destroy(struct hash_table_v1 *hash_table)
{
	...
	rc = pthread_mutex_destroy(&hash_mutex);
	if (rc != 0) {
		exit(rc);
	}
